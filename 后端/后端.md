...
1、 使用 java.lang.reflect 反射包中的 Proxy 类中的。
...
## 2.	ArrayList and LinkList

ArrayList底层使用数组来进行存储，而LinkList使用链表。

LinkList在插入头尾节点会比ArrayList较快，但在中间插入时（add(index, obj)）ArrayList会相对快。查询来说ArrayList会相对快些。
...
这是一个信号量的实现，严格来说不算是一个锁。但是也能够实现和锁相同的功能（资源数量为1）。通过设置一个资源的最大访问数量，若信号量为0，则等待信号释放或等待一段时间后退出。在[例子](#例子)中也有使用到。
...
现在java中的栈：Stack已经不推荐使用了。因为：使用的锁粒度太高了，对性能负载大。推荐使用`DeQueue`；不仅锁粒度小，还能够进行双端操作。基本上有Queue实现的会有对应的DeQueue
