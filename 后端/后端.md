...
​	可以进行服务注册，将所有服务都注册到Consul中心，再通过Consul调用其他服务。
...
Append Only File，在Redis进行修改结构的操作时（set k v）会将该操作记录下来，保存到配置文件中设置的文件中。默认：`appendonlyfile.aof`；在Redis进行重启时，会自动依次运行这些命令。
...
结合nacos使用，nacos是服务注册中心。也就是

#### nacos

只进行发现的话只需要discovery依赖即可。`使用时注意对应spring cloud alibaba版本！` 。
